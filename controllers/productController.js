...
const getValue = (row, keys) => {
  ...
  for (let searchKey of keys) {
    ...
    const foundKey = rowKeys.find(
      (k) => k.trim().toLowerCase() === searchKey.trim().toLowerCase()
    );
    if (foundKey) {
      return row[foundKey];
    }
  ...
  try {
    ...
    const certificationData = rows.map((row, index) => {
      ...
      return {
        ...
        reverse: getValue(row, ["Reverse", "reverse", "REVERSE"]),
        grade: getValue(row, ["Grade", "grade", "GRADE"]),
        price: cleanPrice(getValue(row, ["Price", "price", "PRICE", "Sale Price"])),
        scPrice: cleanPrice(getValue(row, ["SC Price", "sc price", "SC PRICE", "SCPrice"])),
      };
    });
...
// Fetch all certifications (Gold Data)
