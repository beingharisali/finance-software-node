...
//   }
// };
// controllers/uploadCSV.js

const Transaction = require("../models/Transaction");
const CategoryRule = require("../models/CategoryRule");  // ⭐ REQUIRED
const XLSX = require("xlsx");
const moment = require("moment");
...
exports.uploadCSV = async (req, res) => {
  try {
    ...
    // -------------------------
    // PARSE & SANITIZE RECORDS
    // --------------------------
    for (let r of rows) {
      let dateField = r["Transaction Date"];
      let dateValue;

      if (!dateField || dateField.toString().trim() === "") continue;
      ...
      if (r["Debit Amount"] && r["Debit Amount"].toString().trim() !== "") {
        amount = -Math.abs(Number(r["Debit Amount"]));
      } else if (r["Credit Amount"] && r["Credit Amount"].toString().trim() !== "") {
        amount = Math.abs(Number(r["Credit Amount"]));
      } else {
        continue;
      }
      ...
      // ⭐ APPLY AUTO CATEGORY RULE (while reading CSV)
      const rule = await CategoryRule.findOne({
        description: newRecord.transactionDescription.trim(),
      });

      if (rule) {
        newRecord.category = rule.category;
      }
    ...
    for (let record of formattedRecords) {
      // ⭐ Apply rule again (in case)
      const rule = await CategoryRule.findOne({
        description: record.transactionDescription.trim(),
      });
    ...
    return res.status(200).json({
      message:
        duplicates.length > 0
          ? "Some transactions already existed and were skipped."
          : "All transactions uploaded successfully.",
      totalSaved: toInsert.length,
      duplicates: duplicates.length,
    });
